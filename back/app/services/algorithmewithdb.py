...
def run_grouping_algorithm(students_data, preferences_data, n):
    ...
    try:
        ...
        preferences = {}
        for student_id, preferred_ids in preferences_data.items():
            if student_id in student_id_to_name:
                ...
                for pref_id in preferred_ids:
                    if pref_id in student_id_to_name:
                        preferences[student_name].append(student_id_to_name[pref_id])
        ...
        objective = (
            preference_weight * lpSum(z[i, j, g] 
                                    for i in student_names 
                                    for j in preferences.get(i, []) 
                                    for g in range(num_groups))
            + lpSum(level_weights[student_info[i]['level']] * x[i, g]
                   for i in student_names 
                   for g in range(num_groups))
        )
